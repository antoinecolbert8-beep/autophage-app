...
import { redisConnection } from '../queue/social-queue';
import { generateContentWithGemini } from '../gemini-content';
import { enqueueSocialPost } from '../queue/social-queue';
...
export const evidenceQueue = new Queue('evidence-broadcaster', {
...
export const evidenceWorker = new Worker(
...
export async function triggerEvidenceBroadcast(data: { plan: string; amount: number; customerEmail?: string }) {
    ...
    await evidenceQueue.add('broadcast', data, { jobId });
