...
import { ArrowLeft, Rocket, ShieldCheck, Zap, ArrowRight, Wallet, Copy, CheckCircle } from "lucide-react";
...
export default function PartnersPage() {
    ...
    useEffect(() => {
        const id = localStorage.getItem('ela_user_id');
        if (id) {
            setUserId(id);
            fetchStats(id);
        }
    ...
    const fetchStats = async (id: string) => {
        setLoading(true);
        try {
            const res = await fetch(`/api/partners/stats?userId=${id}`);
            ...
            <main className="pt-32 pb-24 px-6">
                <div className="max-w-6xl mx-auto">
                    ...
                    {userId && stats ? (
                        ...
                        <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-1000">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                                <div className="p-8 rounded-3xl bg-white/5 border border-white/10 text-center">
                                    <div className="text-[10px] font-black text-gray-600 uppercase tracking-widest mb-4">REVENU MENSUEL ESTIMÉ</div>
                                    <div className="text-4xl font-black text-[#66fcf1]">{stats.monthlyIncome.toFixed(2)}€</div>
                                </div>
                                <div className="p-8 rounded-3xl bg-white/5 border border-white/10 text-center">
                                    <div className="text-[10px] font-black text-gray-600 uppercase tracking-widest mb-4">FILLEULS ACTIFS</div>
                                    <div className="text-4xl font-black text-white">{stats.activeCount}</div>
                                </div>
                                <div className="p-8 rounded-3xl bg-white/5 border border-white/10 text-center">
                                    <div className="text-[10px] font-black text-gray-600 uppercase tracking-widest mb-4">EN ATTENTE</div>
                                    <div className="text-4xl font-black text-gray-500">{stats.pendingCount}</div>
                                </div>
                                <div className="p-8 rounded-3xl bg-[#66fcf1] text-[#0b0c10] text-center">
                                    <div className="text-[10px] font-black uppercase tracking-widest mb-4">CODE PARTENAIRE</div>
                                    <div className="text-2xl font-black tracking-widest">{stats.referralCode}</div>
                                </div>
                            </div>

                            <div className="bg-[#0f1115] border border-white/5 rounded-[40px] p-8 md:p-12">
                                <h3 className="text-xl font-black text-white mb-8 uppercase tracking-tighter">VOTRE LIEN DE TRACKING SOUVERAIN</h3>
                                <div className="flex flex-col md:flex-row gap-4">
                                    <div className="flex-1 bg-black/50 border border-white/10 rounded-2xl p-6 font-mono text-sm text-[#66fcf1] break-all">
                                        {window.location.origin}/signup?ref={stats.referralCode}
                                    </div>
                                    <button
                                        onClick={copyLink}
                                        className="px-8 py-6 bg-white text-black rounded-2xl font-black text-xs uppercase tracking-[0.2em] hover:bg-[#66fcf1] transition-all flex items-center justify-center gap-2"
                                    >
                            ...
                            {stats.referrals.length > 0 && (
                                <div className="bg-white/5 border border-white/10 rounded-[40px] overflow-hidden">
                                    <table className="w-full text-left text-[10px] uppercase font-black tracking-widest">
                                        <thead className="bg-white/5 text-gray-600">
                                            <tr>
                                                <th className="p-6">FILLEUL (OBSCURCI)</th>
                                        ...
                                        <tbody className="divide-y divide-white/5">
                                            {stats.referrals.map((ref: any, i: number) => (
                                                <tr key={i} className="hover:bg-white/5">
                                                    <td className="p-6 text-white">{ref.email}</td>
                                                    <td className="p-6">{ref.plan}</td>
                                                    <td className="p-6">
                                                        <span className={ref.status === 'active' ? 'text-green-400' : 'text-yellow-400'}>
                                                            {ref.status}
                                                        </span>
                                                    </td>
                                                    <td className="p-6 text-gray-500">{new Date(ref.date).toLocaleDateString()}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
                        ...
                        /* Static landing page for non-partners */
                        <>
                            {/* Hero Section */}
