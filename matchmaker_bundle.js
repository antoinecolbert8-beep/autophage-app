...
// netlify/functions/matchmaker-cron.ts
var matchmaker_cron_exports = {};
__export(matchmaker_cron_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(matchmaker_cron_exports);
...
var handler = schedule("0 9 * * *", async (event) => {
  console.log("\u23F0 Netlify Cron: Triggering Proactive Matchmaker...");
  const siteUrl = process.env.URL || "http://localhost:3000";
  try {
    const response = await (0, import_node_fetch.default)(`${siteUrl}/api/cron/matchmaker`, {
      method: "GET",
      headers: {
    ...
    return {
      statusCode: 200,
      body: JSON.stringify({ message: "Job ex\xE9cut\xE9 avec succ\xE8s.", result: data })
    };
  } catch (error) {
    console.error("\u274C Erreur lors de l'ex\xE9cution du cron:", error);
    return {
      statusCode: 500,
      body: JSON.stringify({ message: "Erreur d'ex\xE9cution", error: error.message })
    };
  }
});
